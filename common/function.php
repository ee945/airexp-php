...
/*
 * 权限控制函数解释：
 * 1. 权限在页面上显示为一组复选框，选中即拥有该权限
 * 2. 权限复选框按顺序对应一串二进制数，转化为十进制数字后作为权限值存入数据库
 * 3. 通过函数ifpermit
 * 4. 函数getpermit
 * 5. 函数getchecked将数据库中的值转成html属性，已显示现有权限
 */
 
// 权限控制函数1：判断是否有某权限，将数据库中权限值转化为对应的0或1，判断是否有该权限
...
// 权限控制函数2：通过复选框获取权限数组，转化为十进制数字权限值
...
// 权限控制函数3：将数据库中权限值转成html复选框属性，已显示现有权限
function getchecked($prm,$i){
    if(ifpermit($prm,$i)){return "checked";}
}

// 以下均为数字金额格式调整函数（来自网络），包括转英文，转大小写，千分位分割等。（曾用于打印结算单，现结算单功能已停用）
function fmoney($num) {
    $num=0+$num;
    $num = sprintf("%.02f",$num);
